...
for id, url in pairs(Games) do
    -- Extract game name from comment or URL
    local name = url:match("GamesScripts/(.-)%.lua") or tostring(id)
...
if not URL then
    ...
    Label.Text = table.concat({
        ...
        '<font size="18" color="rgb(140,140,140)">You will be disconnected in 3 seconds...</font>',
    }, "\n")
    Label.Parent = Frame
...
print("✔ Loading script for game: " .. tostring(GameId))
